...
object ShellSwipeUtil {
    ...
    fun Rect.clickInBounds(durationMs: Long = -1, isWebview: Boolean = false): Boolean {
        ...
        val clickDownUpDiffHappen = Random.nextBoolean()
        ...
        // 生成起始坐标（浮点数）
        val startX = generateRandomFloatInRange(safeLeft.toFloat(), safeRight.toFloat())
        val startY = generateRandomFloatInRange(safeTop.toFloat(), safeBottom.toFloat())
