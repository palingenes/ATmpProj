...
import com.cymf.keyshot.ktx.show
...
//        runMain { AssistsWindowManager.touchableByAll() }
     ...
     * 模拟正常人类退出/杀死应用行为
     */
    suspend fun exitAppHuman(pkgName: String) {
//        YLLogger.e("exitAppHuman $pkgName < ${TaskPollingManager.getCurrTask()?.packageName}")
        recentApps()    //  打开任务栏
        delay(Random.nextInt(1500, 2600).toLong()) //  模拟人停顿时间，同时也是等待动画结束
        ShellSwipeUtil.swipeUpFromBottom(App.app, 0.6f, 0.3f, 60)    // 把 当前应用 移除任务栈
        delay(1500)
        if (Random.nextBoolean()) {
            ShellSwipeUtil.swipeLeftFromRight(App.app, 0.5f, 80)    // 把 当前应用 移除任务栈
        } else {
            ShellSwipeUtil.swipeRightFromLeft(App.app, 0.5f, 80)    // 把 当前应用 移除任务栈
        }
//        ProcessKiller.killApp(App.context, pkgName)
        ...
        AppUtil.randomAdjustAllVolume()
        AppUtil.randomAdjustScreenBrightness()
    }
     ...
     * 尝试打开指定包名的应用
     * 如果本机没有该应用 → 返回 0
     * 打开失败（如 Activity 未找到）→ 返回 -1
     * 正常打开 → 返回 1
     */
    fun launchApp(context: Context, packageName: String): Int {
        return try {
            val intent =
                context.packageManager.getLaunchIntentForPackage(packageName) ?: return 0 // 包不存在
...
//            YLLogger.i("🚚dumpsys window windows | grep -E 'mCurrentFocus|mFocusedApp'\n $foundLine")
    ...
    /**
     * 从当前节点开始，递归收集所有子节点的信息
     * - 只有 ViewGroup（如 LinearLayout、ConstraintLayout）才继续递归
     * - TextView、ImageView、Button 等叶子节点不深入
     * - 最大递归深度：3 层
     */
    fun AccessibilityNodeInfo?.collectAllNodeInfo(maxDepth: Int = 3): List<NodeInfoData> {
        val result = mutableListOf<NodeInfoData>()
        ...
        while (!stack.isEmpty()) {
            ...
            if (depth < maxDepth && shouldTraverseChildren(node)) {
                for (i in node.childCount - 1 downTo 0) {
                    val child = try {
                        node.getChild(i)
                    } catch (_: Exception) {
